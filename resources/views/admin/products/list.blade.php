                            ...
                            <form action="{{route($actionList)}}" method="get" class="form-delete">
                                <div class="row">
                                    <div class="col-md-3 form-group">
                                        <input type="text" name="name" class="form-control" value="{{ isset($filter['name']) ? $filter['name'] : '' }}" placeholder="Điền tên sản phẩm ...">
                                    </div>
                                    <div class="col-md-3 form-group">
                                    ...
                                    <div class="col-md-2 form-group">
                                        <select name="hot" id="status" class="form-control">
                                            <option value=""> -- HOT --</option>
                                            <option value="{{$statusOn}}"  {{ (isset($filter['hot']) && (int)$filter['hot'] == $statusOn ) ? 'selected="selected"' : '' }}>Có</option>
                                            <option value="{{$statusOff}}" {{ (isset($filter['hot']) && (int)$filter['hot'] == $statusOff ) ? 'selected="selected"' : '' }}>Không</option>
                                        </select>
                                    </div>
                                    <div class="col-md-2 form-group">
                            ...
                            <table id="example2" class="table table-bordered table-hover">
                                ...
                                @foreach($datas as $item)
                                    <tr class='{{($item->status == 0)? 'text-danger' : 'text-success'}}'>
                                        ...
                                        <td>
                                            @if($item->hot == $statusOn)
                                                <i class="fa fa-star fa-2x" style="color: darkorange;"></i>
                                            @else
